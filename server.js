...
app.post("/", async (req, res) => {
  ...
  try {
    ...
    if (action === "INIT" || action === "data_exchange") {
      ...
      // --- CRITICAL FIX: TRANSITION TO NEXT SCREEN ---
      if (data?.action === "GO_TO_FINISH") {
        console.log("🚀 BREAKING LOOP: Moving to SUMMARY_SCREEN");
        return res.status(200).send(encryptResponse({
          version: "7.1",
          screen: "SUMMARY_SCREEN", // <--- CHANGING THE ID MOVES THE FLOW FORWARD
    ...
    if (action === "complete") {
      console.log(`✅ COMPLETE RECEIVED! ID: ${data.parishad_id}`);
