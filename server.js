...
// ✅ NEW: Add health check for browser
app.get("/", (req, res) => {
  res.status(200).json({ status: "active", message: "ABTYP Flow Server is Ready" });
});
...
app.post("/", async (req, res) => {
  ...
  try {
    aesKey = crypto.privateDecrypt({ key: PRIVATE_KEY, padding: crypto.constants.RSA_PKCS1_OAEP_PADDING, oaepHash: "sha256", mgf1Hash: "sha256" }, Buffer.from(encrypted_aes_key, "base64"));
    ...
    if (action === "INIT" || action === "data_exchange") {
      if (data?.action === "GO_TO_DETAILS") {
        return res.status(200).send(encryptResponse({
          ...
          data: { prev_data: { name: data.full_name, mobile: data.mobile, age: data.age, gender: data.gender, p_id: data.parishad_id } }
        }, aesKey, requestIv));
      }
      ...
      if (isDeathFlow) resp.data.gender_list = [{id: "Male", title: "Male"}, {id: "Female", title: "Female"}];
    ...
    // If we have keys, try to send an encrypted error, otherwise just 200
    if (aesKey && requestIv) {
        return res.status(200).send(encryptResponse({ version: "7.1", data: { error: "Something went wrong" } }, aesKey, requestIv));
    }
