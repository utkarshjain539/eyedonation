...
app.get("/", (req, res) => {
  res.status(200).send("ABTYP Multi-Flow Server Ready");
});
...
app.post("/", async (req, res) => {
  ...
  try {
    ...
    // 🎯 Improved Multi-Flow Detection
    const isDeath = (flow_token?.toLowerCase().includes("death")) || (screen?.toLowerCase().includes("death"));
    const isUserReg = (flow_token?.toLowerCase().includes("reg")) || (screen?.toLowerCase().includes("user_reg"));
    ...
    if (action === "INIT" || action === "data_exchange") {
      // Determine which screen ID to return
      ...
      // Add gender list for both Death and User Registration flows
      if (isDeath || isUserReg) {
    ...
    if (aesKey && requestIv) {
      return res.status(200).send(encryptResponse({ version: "7.1", data: { error: "Unexpected error occurred" } }, aesKey, requestIv));
    }
